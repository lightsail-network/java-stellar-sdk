...
public class LiquidityPoolsPageDeserializerTest extends TestCase {
    ...
    public void testDeserialize() {
        Page<LiquidityPoolResponse> liquidityPoolsPage = GsonSingleton.getInstance().fromJson(json, new TypeToken<Page<LiquidityPoolResponse>>() {}.getType());

        assertEquals("https://horizon.stellar.org/liquidity_pools?cursor=3748308153536513-0\\u0026limit=10\\u0026order=asc", liquidityPoolsPage.getLinks().getNext().getHref());
        assertEquals("https://horizon.stellar.org/liquidity_pools?cursor=3697472920621057-0\\u0026limit=10\\u0026order=desc", liquidityPoolsPage.getLinks().getPrev().getHref());
        assertEquals("https://horizon.stellar.org/liquidity_pools?cursor=\\u0026limit=10\\u0026order=asc", liquidityPoolsPage.getLinks().getSelf().getHref());
        ...
        assertEquals("https://horizon.stellar.org/liquidity_pools/67260c4c1807b262ff851b0a3fe141194936bb0215b2f77447f1df11998eabb9/effects{?cursor,limit,order}", liquidityPool.getLinks().getEffects().getHref());
        assertEquals("https://horizon.stellar.org/liquidity_pools/67260c4c1807b262ff851b0a3fe141194936bb0215b2f77447f1df11998eabb9/operations{?cursor,limit,order}", liquidityPool.getLinks().getOperations().getHref());
        assertEquals("https://horizon.stellar.org/liquidity_pools/67260c4c1807b262ff851b0a3fe141194936bb0215b2f77447f1df11998eabb9", liquidityPool.getLinks().getSelf().getHref());
        assertEquals("https://horizon.stellar.org/liquidity_pools/67260c4c1807b262ff851b0a3fe141194936bb0215b2f77447f1df11998eabb9/transactions{?cursor,limit,order}", liquidityPool.getLinks().getTransactions().getHref());

        assertEquals(new LiquidityPoolID("14be5a5b3d3f5e1e74380ab0a3bf9c172b7246fdf7753b172cbacd4d66143c08"), liquidityPoolsPage.getRecords().get(1).getID());
    }
