...
class FederationTest :
    FunSpec({
        ...
        test("resolveAddress success") {
            ...
            federationRequest.path shouldBe "/federation?type=name&q=bob*${URLEncoder.encode(domain, "UTF-8")}"
        ...
        test("malformed address throws exception") {
            shouldThrow<IllegalArgumentException> {
                Federation().resolveAddress("bob*stellar.org*test")
            }
            shouldThrow<IllegalArgumentException> {
                Federation().resolveAddress("bob")
            }
        }
        ...
        test("no federation server in stellar.toml throws exception") {
            ...
            shouldThrow<NoFederationServerException> {
                federation().resolveAddress("bob*$domain")
            }
        }
        ...
        test("federation server using http throws exception") {
            ...
            shouldThrow<FederationServerInvalidException> {
                federation().resolveAddress("bob*$domain")
            }
        }
    })
