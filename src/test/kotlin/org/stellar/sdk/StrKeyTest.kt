  ...
  FunSpec({
    context("ACCOUNT_ID") {
      ...
      test("is valid") {
        ...
        validPublicKeys.forEach { publicKey ->
          StrKey.isValidEd25519PublicKey(publicKey) shouldBe true
        }

        val invalidPublicKeys =
          listOf(
    ...
    context("SEED") {
      ...
      test("should validate valid seeds") {
        val validSeeds =
          listOf(
      ...
      test("should reject invalid seeds") {
        val invalidSeeds =
          listOf(
    ...
    context("SIGNED_PAYLOAD") {
      ...
      test("is valid") {
        ...
        invalidKeys.forEach { key -> StrKey.isValidSignedPayload(key) shouldBe false }
      }

      // TODO: add encode check
    }

    test("should reject all invalid StrKey cases") {
      ...
      invalidKeys.forEach { key ->
        shouldThrow<IllegalArgumentException> {
          ...
          StrKey.decodeEd25519PublicKey(key)
          StrKey.decodeMed25519PublicKey(key)
          StrKey.decodeEd25519SecretSeed(key.toCharArray())
          StrKey.decodePreAuthTx(key)
          StrKey.decodeSha256Hash(key)
          StrKey.decodeSignedPayload(key)
          StrKey.decodeContract(key)
          StrKey.decodeLiquidityPool(key)
          StrKey.decodeClaimableBalance(key)
        }
      }
    }
